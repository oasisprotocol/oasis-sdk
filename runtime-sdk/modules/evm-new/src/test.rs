...
fn test_transfer_event() {
    ...
    // XXX: Give Dave some tokens to pay for fees.
    Accounts::mint(
        keys::dave::address(),
        &token::BaseUnits(1_000_000, Denomination::NATIVE),
    )
    .unwrap();
    ...
    println!("###1 {:?}", dispatch_result.result);
    assert!(dispatch_result.result.is_success(), "create should succeed");
    let result = dispatch_result.result.unwrap();
    let result: Vec<u8> = cbor::from_value(result).unwrap();
    let contract_address = H160::from_slice(&result);
    let contract_address_native = EVMConfig::map_address(contract_address.0.into());

    // Give the faucet some tokens.
    println!(
        "*** Balance before mint: {:?}",
        Accounts::get_balance(contract_address_native, Denomination::NATIVE)
    );
    let result = Accounts::mint(
    ...
    println!("###3 {:?}", dispatch_result.result);
    assert!(
        dispatch_result.result.is_success(),
        "withdraw call should succeed"
    );
