...
impl<Consensus: modules::consensus::API> Module<Consensus> {
    ...
    fn internal_shares_to_tokens<C: Context>(
        ...
        let tokens = args
            .shares
            .checked_mul(
                u128::try_from(pool.balance.clone()).expect("pool balance should fit into u128"),
            )
            .expect("shares * total_amount should not overflow")
            .checked_div(
                u128::try_from(pool.total_shares.clone())
                    .expect("pool total_shares should fit into u128"),
            )
