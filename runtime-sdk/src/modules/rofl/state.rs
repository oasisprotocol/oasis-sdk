...
pub struct KeyEndorsementInfo {
    ...
    /// Identifier of the app this key is for.
    pub app_id: AppId,
}
...
#[derive(Clone, Debug, Default, PartialEq, Eq, cbor::Encode, cbor::Decode)]
#[cbor(as_array)]
struct LegacyKeyEndorsementInfo {
    /// Identifier of node that endorsed the enclave.
    pub node_id: CorePublicKey,
    /// RAK of the enclave that endorsed the key. This is only set for endorsements of extra keys.
    pub rak: Option<CorePublicKey>,
...
impl KeyEndorsementInfo {
    ...
    pub fn for_rak(node_id: CorePublicKey, app_id: AppId) -> Self {
        Self {
            node_id,
            app_id,
            ..Default::default()
    ...
    pub fn for_extra_key(node_id: CorePublicKey, app_id: AppId, rak: CorePublicKey) -> Self {
        Self {
            node_id,
            rak: Some(rak),
            app_id,
...
impl From<LegacyKeyEndorsementInfo> for KeyEndorsementInfo {
    fn from(value: LegacyKeyEndorsementInfo) -> Self {
        Self {
            node_id: value.node_id,
            rak: value.rak,
            // Using a default app ID is fine because this app ID cannot be generated by any scheme.
...
pub fn update_registration(registration: types::Registration) -> Result<(), Error> {
    ...
    CurrentState::with_store(|mut root_store| {
        ...
        endorsers.insert(
            hrak,
            KeyEndorsementInfo::for_rak(registration.node_id, registration.app),
        );

        for pk in &registration.extra_keys {
            endorsers.insert(
                hash_pk(pk),
                KeyEndorsementInfo::for_extra_key(
...
pub fn get_endorser(pk: &PublicKey) -> Option<KeyEndorsementInfo> {
    ...
    CurrentState::with_store(|store| {
        ...
        let kei: Option<cbor::Value> = endorsers.get(hpk);
        kei.map(|kei| match kei {
            cbor::Value::Array(ref items) if items.len() == 2 => {
                // Legacy variant without app ID.
                let legacy: LegacyKeyEndorsementInfo = cbor::from_value(kei).unwrap();
                legacy.into()
            }
            _ => cbor::from_value(kei).unwrap(),
        })
    })
}
...
mod test {
    ...
    fn test_legacy_kei() {
        let _mock = mock::Mock::default();
        let rak_pk = keys::alice::pk();
        let hpk = hash_pk(&rak_pk);

        CurrentState::with_store(|store| {
            let store = storage::PrefixStore::new(store, &MODULE_NAME);
            let mut endorsers =
                storage::TypedStore::new(storage::PrefixStore::new(store, &ENDORSERS));
            endorsers.insert(
                hpk,
                LegacyKeyEndorsementInfo {
        ...
        let kei = get_endorser(&rak_pk).unwrap();
        assert_eq!(kei.node_id, keys::bob::pk_ed25519().into());
        assert!(kei.rak.is_none());
        assert_eq!(kei.app_id, AppId::default());
    }
}
