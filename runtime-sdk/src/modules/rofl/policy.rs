...
impl AppAuthPolicy {
    /// Number of atoms in the endorsement policy.
    pub fn endorsement_atom_count(&self) -> usize {
        self.endorsements
            .iter()
            .fold(0, |acc, atom| acc + atom.atom_count())
    }
}
...
pub enum AllowedEndorsement {
    ...
    Provider(Address),
    /// Any provider machine where the given address is currently the admin.
    #[cbor(rename = "provider_machine_admin")]
    ProviderMachineAdmin(Address),
...
impl AllowedEndorsement {
    /// Number of atoms in this endorsement policy.
    pub fn atom_count(&self) -> usize {
        match self {
            Self::And(atoms) | Self::Or(atoms) => {
                atoms.iter().fold(1, |acc, atom| acc + atom.atom_count())
            }
            _ => 1,
        }
    }
