...
pub trait API {
    ...
    fn create<C: TxContext>(ctx: &mut C, init_code: Vec<u8>) -> Result<Vec<u8>, Error>;

    /// Perform an Ethereum CALL transaction.
    fn call<C: TxContext>(ctx: &mut C, address: H160, data: Vec<u8>) -> Result<Vec<u8>, Error>;
...
impl API for Module {
    fn create<C: TxContext>(ctx: &mut C, init_code: Vec<u8>) -> Result<Vec<u8>, Error> {
        ...
        Self::do_evm(caller, ctx, |exec| {
            let address = exec.create_address(evm::CreateScheme::Legacy { caller });
            (
                exec.transact_create(caller, U256::zero(), init_code, u64::max_value()),
    ...
    fn call<C: TxContext>(ctx: &mut C, address: H160, data: Vec<u8>) -> Result<Vec<u8>, Error> {
        let caller = Self::derive_caller(ctx)?;
        Self::do_evm(caller, ctx, |exec| {
            exec.transact_call(caller, address, U256::zero(), data, u64::max_value())
        })
    }
...
impl Module {
    ...
    fn tx_create<C: TxContext>(ctx: &mut C, body: types::CreateTx) -> Result<Vec<u8>, Error> {
        Self::create(ctx, body.init_code)
    }

    fn tx_call<C: TxContext>(ctx: &mut C, body: types::CallTx) -> Result<Vec<u8>, Error> {
        Self::call(ctx, H160::from_slice(&body.address), body.data)
    }
