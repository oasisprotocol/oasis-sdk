...
pub fn decode_call<C: Context>(
    ...
    match call.format {
        ...
        CallFormat::EncryptedX25519DeoxysII => {
            ...
            // TODO: Get transaction key pair from the key manager.
            let (_, sk) = deoxysii::generate_key_pair();
            // Derive shared secret via X25519 and open the sealed box.
            let data =
                deoxysii::box_open(&envelope.nonce, envelope.data, vec![], &envelope.pk, &sk)
...
pub fn encode_result<C: Context>(
    ...
    match metadata {
        ...
        // Encrypted data format using X25519 key exchange and Deoxys-II symmetric encryption.
        Metadata::EncryptedX25519DeoxysII { pk, index } => {
            ...
            // Seal the result.
            let data = deoxysii::box_seal(&nonce, result, vec![], &pk, &sk).unwrap();

            CallResult::Unknown(data)
        }
    }
}
