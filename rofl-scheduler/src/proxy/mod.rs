...
use self::domain::{CustomDomainVerifier, DomainVerificationHandle};
use crate::{
    config::ProxyConfig,
    proxy::domain::CustomDomainVerifierNotifier,
    types::{domain_verification_token, METADATA_KEY_PROXY_CUSTOM_DOMAINS},
};

/// Maximum number of custom domains that can be configured at the same time.
const MAX_CUSTOM_DOMAINS: usize = 1;
...
struct InstanceInfo {
    ...
    #[allow(dead_code)]
    domain_verification_handle: Arc<DomainVerificationHandle>,
}
...
impl Proxy {
    ...
    pub async fn provision_instance(
        ...
        let domain_verification_handle = DomainVerificationHandle::new();
    ...
    fn extract_custom_domains(&self, deployment: &Deployment) -> Vec<String> {
...
#[async_trait]
impl CustomDomainVerifierNotifier for ProxyDomainVerificationHandler {
