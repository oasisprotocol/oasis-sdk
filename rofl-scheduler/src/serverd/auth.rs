    ...
    pub async fn verify(&self, domain: &str, provider: Address) -> Result<Claims, anyhow::Error> {
        match self {
            Self::Siwe { message, signature } => {
                ...
                let exp = expiry.timestamp() as u64;
                let address = Address::from_eth(&message.address);
            ...
            Self::Std { body, signature } => {
                ...
                let expiry = Utc::now() + Duration::hours(6);
                let exp = (expiry.timestamp() as u64).min(body.not_after);
                let address = Address::from_sigspec(
