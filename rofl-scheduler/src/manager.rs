...
impl Manager {
    ...
    async fn discover(&self, local_node_id: PublicKey) -> Result<LocalState> {
        ...
        let client = self.client.latest_round().await?;
        ...
        local_state.running = rsp
            ...
            .collect();
        let mut running_unknown = BTreeSet::from_iter(local_state.running.keys());
    ...
    async fn pull_and_deploy_instance(
        ...
        let (temporary_name, manifest_hash) = self
            .pull_and_validate_deployment(instance, deployment)
            .await?;
        self.deploy_instance(instance, deployment, temporary_name, manifest_hash)
            .await
    }
    ...
    async fn pull_and_validate_deployment(
        ...
        // TODO: Support ORCs with multiple components when we know how to handle persistent storage.
        ...
        for (idx, component) in orc_manifest.components.iter_mut().enumerate() {
            // Rewrite component name into its index to ensure persistent storage consistency. The
            // host will ensure this gets separated by origin and instance ID.
        ...
        Ok((temporary_name, new_orc_manifest_hash))
    }
