...
async fn maybe_start<A: App>(
    ...
    // Store the proxy domain in an environment variable so that the containers can use it in
    // their configuration.
    containers::env().set(PROXY_HOST_ENV_NAME, &proxy_label.http_host);
...
async fn run(proxy_label: ProxyLabel, compose: ParsedCompose) -> Result<()> {
    ...
    let wg_cfg = wireguard::ClientConfig {
        listen_port: Some(wireguard::WG_DEFAULT_LISTEN_PORT),
    ...
    for mapping in compose.port_mappings {
        let name = format!("p{}.{}", mapping.port.host_port, proxy_label.http_host);
