...
export class HDKey implements Signer {
     ...
     */
    public static async generateValidMnemonic(strength = 256): Promise<string> {
        let mnemonic = generateMnemonic(strength);
        while (!(await HDKey.isValidMnemonic(mnemonic))) {
     ...
     * @param mnemonic
     */
    public static async isValidMnemonic(mnemonic: string): Promise<boolean> {
        const seed = await mnemonicToSeed(mnemonic);
    ...
    public static fromSeed(seed: Uint8Array): HDKey {
        if (seed.length != 32) throw 'Seed must be 32 bytes (a Uint8Array of size 32)';
    ...
    public deriveChild(index: BN): HDKey {
        ...
        if (index.lt(bn(2).pow(bn(31)))) {
            ...
            c = this.hmacSha512(concat(new Uint8Array([3]), new Uint8Array(AP), i_bytes), cP).slice(
                32,
            );
        } else {
            // hardened child
