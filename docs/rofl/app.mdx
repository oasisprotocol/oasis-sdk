...
CLI. We also need to select the network (in this case `testnet`) and the account
...
Anyone with enough funds can create an app. Currently, this threshold is 10,000
...
Passing the `--update-manifest` argument will make the CLI automatically update
the app manifest (`rofl.yaml`) with the assigned app identifier.
...
In contrast to Docker compose files, always use fully qualified domain name
when specifying the image URL, e.g. `docker.io/ollama/ollama` and not just
`ollama/ollama`.
